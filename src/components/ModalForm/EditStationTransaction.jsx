...
const EditStationTransaction = ({ row }) => {
      ...
      try {
        ...
        if (res.status != 200) {
          ...
          const matchedItem = res.data.find(
            (item) => String(formData.fuelman_id) === item.JDE
          );
          if (matchedItem) {
  ...
  const handleChange = (selectedOption) => {
    if (selectedOption) {
      ...
      const matchedItem = userData.find(
        (item) => String(selectedOption.value) === item.JDE
      );
      if (matchedItem) {
        setUserName(matchedItem.fullname);
      } else {
  ...
  const handleEditData = async () => {
    ...
    try {
      const normalizedDate = normalizeDateToYYYYMMDD(formData.date);
      ...
      updateList.date = normalizedDate;
