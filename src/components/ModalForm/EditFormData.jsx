...
const ModalFormDataEdit = ({ row }) => {
  ...
  const handleChangeEnd = (time) => {
    ...
    setFormData((prevFormData) => {
      if (
        prevFormData.start &&
        moment(formattedEnd, "HH:mm:ss").isBefore(
          moment(prevFormData.start, "HH:mm:ss")
        )
      ) {
        setErrors({
  ...
  const handleOptionChange = (value) => {
    ...
    const filteredData = storedTrx.filter(
      (item) => item.type === "Transfer" || item.type === "Issued"
    );
  ...
  const handleValidation = () => {
    ...
    if (!formData.jde_operator)
      newErrors.jde_operator = "Jde operator is required";
  ...
  const handleSubmitData = async () => {
    ...
    if (!isValid) {
      ...
      try {
        ...
        const dates = JSON.parse(localStorage.getItem("tanggal"));
        const res = await formService.updateData({
          id: row.id,
        ...
        setEditStatus("Error");
        setEditMessage(
          "Terjadi kesalahan saat update data. Data tidak tersimpan!"
        );
      } finally {
        showEditModal();
      }
  ...
  const getSelectedUnit = (no_unit) => {
    ...
    return matchedUnit
      ? { label: matchedUnit.unit_no, value: matchedUnit.unit_no }
      : null;
  };
        ...
        >
          ...
          <EuiModalBody>
            <EuiForm id={modalFormId} component="form">
              <EuiFlexGrid columns={2}>
                <EuiFormRow
                  label="No Unit"
                  isInvalid={!!errors.no_unit}
                  error={errors.no_unit}
                >
                  <CreatableSelect
                    styles={customStyles}
                    options={options}
                ...
                <EuiFormRow
                  label="HM/KM Transaksi Terakhir"
                  style={{ marginTop: "0px" }}
                >
                  <EuiFieldText
                    name="hm_last"
                ...
                <EuiFormRow
                  label="Flow Meter Awal"
                  style={{ marginTop: "0px" }}
                  isInvalid={!!errors.flow_start}
                  error={errors.flow_start}
                >
                  <EuiFieldText
                    name="flow_start"
                ...
                <EuiFormRow
                  label="Flow Meter Akhir"
                  style={{ marginTop: "0px" }}
                  isInvalid={!!errors.flow_end}
                  error={errors.flow_end}
                >
                  <EuiFieldText
                    name="flow_end"
                ...
                <EuiFormRow
                  label="Employee Id"
                  isInvalid={!!errors.jde_operator}
                  error={errors.jde_operator}
                >
                  <CreatableSelect
                    styles={customStyles}
                    options={optJde}
          ...
          <EuiModalFooter>
            <EuiButton
              ...
              onClick={() => {
                document
                  .getElementById(modalFormId)
                  ?.dispatchEvent(new Event("submit")); // Trigger form submission
                closeModal();
              }}
              fill
            >
      ...
      {isEditResult && (
        <EuiModal>
          <EuiModalBody>
            ...
            >
              {editStatus === "Success!"
                ? "Data berhasil terupdate.  Silahkan kembali untuk menambah data atau ke halaman utama."
